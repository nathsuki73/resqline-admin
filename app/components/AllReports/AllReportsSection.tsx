...
const mapApiReportToIncidentReport = (report: any): IncidentReport => {
  ...
  return {
    ...
    time: new Date(rawDate).toLocaleTimeString([], {
      hour: "2-digit",
      minute: "2-digit",
    }),
    dateISO: rawDate,
...
export default function AllReportsSection({
  ...
  const [statusFilter, setStatusFilter] = useState<
    "all" | "archived" | IncidentStatus
  >("all");
  const [departmentFilter, setDepartmentFilter] = useState<
    "all" | IncidentDepartment
  >("all");
  ...
  const [draftNotesByReportId, setDraftNotesByReportId] = useState<
    Record<string, string>
  >({});
  const [noteSaveStateByReportId, setNoteSaveStateByReportId] = useState<
    Record<string, "unsaved" | "saving" | "saved">
  >({});
  ...
  useEffect(() => {
    const mapped = Array.isArray(apiReports)
      ? apiReports.map(mapApiReportToIncidentReport)
      : [];

    setReports(
      mapped.map((report) => {
        const syncedNote = getIncidentNoteForId(report.id);
        return syncedNote === null
          ? report
          : { ...report, internalNote: syncedNote };
      }),
    );
  }, [apiReports]);
  ...
  useEffect(() => {
    ...
    setDraftNotesByReportId((prev) => {
      if (Object.prototype.hasOwnProperty.call(prev, activeReportId))
        return prev;
  ...
  useEffect(() => {
    const onIncidentNoteUpdated = (event: Event) => {
      ...
      setReports((prev) =>
        prev.map((report) =>
          report.id === detail.id
            ? { ...report, internalNote: detail.note }
            : report,
        ),
      );
  ...
  const exportCsv = () => {
    ...
    const rows = sortedReports.map((report) => {
      ...
      const reverseGeoCode =
        raw?.reportedAt?.reverseGeoCode ?? raw?.location?.reverseGeoCode;
    ...
    const csv = [header, ...rows]
      .map((row) =>
        row.map((cell) => `"${toText(cell).replaceAll('"', '""')}"`).join(","),
      )
      .join("\n");
  ...
  const openReportDetails = async (reportId: string) => {
    ...
    try {
      ...
      setReports((prev) =>
        prev.map((report) =>
          report.id === reportId ? { ...report, ...fullReport } : report,
        ),
      );
    } catch (error) {
  ...
  const updateFocusedReportInternalNoteDraft = (nextNote: string) => {
    ...
    setDraftNotesByReportId((prev) => ({
      ...prev,
      [activeReportId]: nextNote,
    }));
    setNoteSaveStateByReportId((prev) => ({
      ...prev,
      [activeReportId]: "unsaved",
    }));
  };
  ...
  const saveFocusedReportInternalNote = () => {
    ...
    setNoteSaveStateByReportId((prev) => ({
      ...prev,
      [activeReportId]: "saving",
    }));

    setReports((prev) =>
      prev.map((report) =>
        report.id === activeReportId
          ? { ...report, internalNote: draft }
          : report,
      ),
    );
  ...
  const openDispatchModal = () => {
    ...
    if (
      activeReportId &&
      noteSaveStateByReportId[activeReportId] === "unsaved"
    ) {
  ...
  const handleDispatchFromAllReports = async (
      ...
      prev.map((report) =>
        report.id === reportInFocus.id
          ? {
              ...
              internalNote:
                note.trim().length > 0 ? note.trim() : report.internalNote,
            }
          : report,
      ),
    );
    ...
    if (activeReportId) {
      setDraftNotesByReportId((prev) => ({
        ...prev,
        [activeReportId]:
          note.trim().length > 0 ? note.trim() : (prev[activeReportId] ?? ""),
      }));
      setNoteSaveStateByReportId((prev) => ({
        ...prev,
        [activeReportId]: "saved",
    ...
    if (selectedUnitIds.length > 0) {
      syncActiveIncidentToDashboard({
        ...
        status: "in-progress",
        internalNote:
          note.trim().length > 0 ? note.trim() : reportInFocus.internalNote,
      });
    }
  };
  ...
  const focusedDraftNote = activeReportId
    ? (draftNotesByReportId[activeReportId] ??
      reportInFocus?.internalNote ??
      "")
    : "";

  const focusedNoteSaveState = activeReportId
    ? (noteSaveStateByReportId[activeReportId] ?? "saved")
    : "saved";
  ...
  return (
    <>
      <main className="flex h-screen flex-1 flex-col bg-(--color-bg) p-4">
        <section className="flex min-h-0 flex-1 flex-col rounded-xl border border-(--color-border-1) bg-(--color-surface-1)">
          <header className="flex flex-wrap items-start justify-between gap-3 border-b border-(--color-border-1) px-4 py-3">
            <div>
              <h1 className="text-3xl font-bold leading-tight text-(--color-text-1) md:text-4xl">
                All Reports
              </h1>
              <p className="mt-1 text-[13px] text-(--color-text-3)">
                Complete incident logs: sortable, filterable
              </p>
            </div>
          ...
          <div className="flex flex-wrap items-center gap-2 border-b border-(--color-border-1) px-3 py-2">
            <label className="relative min-w-60 flex-1">
              <Search
                size={14}
                className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-(--color-text-4)"
              />
              <input
                type="text"
                value={searchQuery}
            ...
            <div className="relative">
              ...
              <ChevronDown
                size={12}
                className="pointer-events-none absolute right-2.5 top-1/2 -translate-y-1/2 text-(--color-text-4)"
              />
            </div>
            ...
            <div className="relative">
              <select
                ...
                onChange={(e) => {
                  setDepartmentFilter(
                    e.target.value as typeof departmentFilter,
                  );
                  setCurrentPage(1);
                }}
          ...
          <div className="flex min-h-0 flex-1 overflow-hidden">
            <div className="min-h-0 min-w-0 flex-1 overflow-auto">
              <AllReportsTable
                ...
                getDepartmentClasses={(department) =>
                  getIncidentDepartmentClasses(department)
                }
            ...
            <AllReportsSidebar
              ...
              onChangeStatus={(status) =>
                updateFocusedReportStatus(status as IncidentStatus)
              }
      ...
      <DispatchUnitModal
        ...
        incidentId={
          reportInFocus ? `RPT-2026-${reportInFocus.id}` : "RPT-2026-0000"
        }
        ...
        severity={
          reportInFocus
            ? `${severityLabel[reportInFocus.severity]} - SOS`
            : "Critical - SOS"
        }
