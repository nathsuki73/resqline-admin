...
import { getReportCategoryInput, mapCategoryCodeToDepartment, mapCategoryCodeToType } from "@/app/constants/reportCategories";
import { mapApiStatusToSlug, mapResponderStatusToMobileStatus, type IncidentStatusSlug } from "@/app/constants/reportStatus";
import { type BridgeIncident } from "./incidentBridge";

const severityFromStatus = (status: IncidentStatusSlug): BridgeIncident["severity"] => {
...
const formatLocation = (report: Record<string, unknown>) => {
  const reportedAt = report.reportedAt && typeof report.reportedAt === "object" ? (report.reportedAt as Record<string, unknown>) : null;
  const location = report.location && typeof report.location === "object" ? (report.location as Record<string, unknown>) : null;
...
export const mapApiReportToBridgeIncident = (report: Record<string, unknown>): BridgeIncident => {
  ...
  return {
    ...
    reporter: String(report.reportByName || report.reportedBy?.name || "Unknown"),
    reporterContact: String(report.reportByPhoneNumber || report.reportedBy?.phoneNumber || "No contact provided"),
    department: mapCategoryCodeToDepartment(categoryInput) as BridgeIncident["department"],
    severity: severityFromStatus(status),
    status,
    mobileStatus: mapResponderStatusToMobileStatus(status),
    time: new Date(String(rawDate)).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" }),
    reporterDescription: String(report.description || ""),
    internalNote: String(report.internalNote || ""),
    images: Array.isArray(report.images) ? (report.images as string[]) : Array.isArray(report.image) ? (report.image as string[]) : [],
    type: incidentCategory,
    aiAnalysis: (report.aiProbabilities as BridgeIncident["aiAnalysis"]) ?? undefined,
  };
};

export const fetchBridgeIncidentById = async (reportId: string): Promise<BridgeIncident | null> => {
